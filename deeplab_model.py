...
def deeplab_v3_plus_generator(num_classes,
  ...
  def model(inputs, is_training):
    ...
    #       # logits_reid = slim.flatten(logits_reid, scopre='flatten')
    #       logits = slim.conv2d(logits_reid, 1041, [1, 1], activation_fn=None,
    #                                normalizer_fn=None, scope='output')
    #       # predictions = slim.fully_connected(net, 1, activation_fn=None, scope='output')


    logits_reid = end_points['Logits'][:shape]
    ...
    ##add dilated
    lip_net = end_points['dalated_inception7'][shape:]
      ...
      with tf.contrib.slim.arg_scope(inception_v3_arg_scope()):
        with arg_scope([layers.batch_norm], is_training=is_training):
          with tf.variable_scope("low_level_features"):
            low_level_features = end_points['MaxPool_5a_3x3'][shape:]
          ...
          with tf.variable_scope("upsampling_logits"):
            ...
            # net = layers_lib.conv2d(net, 256, [3, 3], stride=1, scope='conv_3x3_1_1')
            ...
            # logits_reid = layers_lib.conv2d(logits_reid, 1041, [1,1], stride=1, activation_fn=None,
            # normalizer_fn=None,scope='conv_3x3_222')
            # logits = slim.conv2d(logits, 1041, [3,3], activation_fn=None,
